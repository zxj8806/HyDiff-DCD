...
retainedAuthors = set()
authorLabelHistogram = {y:{} for y in range(limitMinYear, limitMaxYear+1)}
labelIds = dict()
authorsNames = dict()
labelKeywords = dict()
labelKeyAll = set()
coAuthoredCount = dict()
...
def extract_name(text):
	pattern = r"<(booktitle|journal)>(.*?)<\/(booktitle|journal)>"
...
def main(argv):
	...
	dblpPathnamePrefix = 'dblp-2019-12-01'


	buildlabelKeywords()

	loadData(dblpPathnamePrefix + ".xml")
